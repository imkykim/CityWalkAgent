...
class ValidationAnalyzer:
    ...
    def analyze_dimension(self, dimension: str) -> Dict[str, Any]:
        ...
        vlm_normalized = normalizer.zscore_normalize(vlm)

        # Secondary metrics: Pearson, MAE, RMSE, R² (after normalization)
        pearson_r, pearson_p = pearsonr(vlm_normalized, knn)
        mae = mean_absolute_error(knn, vlm_normalized)
        rmse = np.sqrt(mean_squared_error(knn, vlm_normalized))
        r2 = r2_score(knn, vlm_normalized)
    ...
    def generate_report(self) -> str:
        ...
        report.append("═" * 70)
        report.append("")
        report.append(f"Dataset: {self.n_samples} images")
        ...
        # Secondary metrics
        report.append("─" * 70)
        report.append("                SECONDARY METRICS (Z-normalized)")
        report.append("─" * 70)
        report.append(
        ...
        report.append("─" * 70)
        report.append(f"{'Dimension':<15} │ {'VLM Mean':<10} │ {'KNN Mean':<10} │ {'VLM Std':<9} │ {'KNN Std':<9}")
        report.append("─" * 15 + "┼" + "─" * 12 + "┼" + "─" * 12 + "┼" + "─" * 11 + "┼" + "─" * 9)

        for _, row in results_df.iterrows():
            report.append(
                f"{row['dimension'].capitalize():<15} │ "
                f"{row['vlm_mean']:>10.2f} │ "
                f"{row['knn_mean']:>10.2f} │ "
                f"{row['vlm_std']:>9.2f} │ "
                f"{row['knn_std']:>9.2f}"
            )
    ...
    def plot_correlation(
        ...
        # Scatter plot
        ax.scatter(knn, vlm, alpha=0.6, s=50, edgecolors="black", linewidths=0.5)

        # Add regression line
        z = np.polyfit(knn, vlm, 1)
        p = np.poly1d(z)
        x_line = np.linspace(knn.min(), knn.max(), 100)
        ax.plot(x_line, p(x_line), "r--", alpha=0.8, linewidth=2, label="Regression")

        # Add diagonal reference line
        min_val = min(knn.min(), vlm.min())
        max_val = max(knn.max(), vlm.max())
        ax.plot(
            [min_val, max_val],
            [min_val, max_val],
            "k:",
        ...
        # Labels and title
        ax.set_xlabel("K-NN Predicted Score", fontsize=12)
        ax.set_ylabel("VLM Score", fontsize=12)
        ax.set_title(
    ...
    def plot_all_dimensions(self, output_dir: Path):
        ...
        for idx, dim in enumerate(self.DIMENSIONS):
            ...
            # Scatter plot
            ax.scatter(knn, vlm, alpha=0.6, s=30, edgecolors="black", linewidths=0.5)
